...
class authModel extends Model {
   ...
   async register(req, res) {
      ...
      if (password.length >= 8 && password === password_confirm) {
         //check unique users
         let check = await User.getUser(login, email);
         ...
         if (rows.length > 0) {
            ...
            await User.addUser(login, hashPass, email, photo, activationLink);
            let check = await User.getUser(login, email);
            let [rows, fields] = check;
            let user = rows[0];
            //generation token
   ...
   async login(req, res) {
      try {
         ...
         let check = await User.loginUser(login);
         let [rows, fields] = check;
         let user = rows[0];
      ...
      try {
         ...
         if (password.length >= 8 && password === password_confirm) {
            ...
            if (rows.length > 0) {
               ...
               const activationLink = uuid.v4();
               //add user data
               await User.addUser_ADMIN(login, hashPass, email, photo, status, verify, activationLink);
               let check = await User.getUser(login, email);
               let [rows, fields] = check;
               let user = rows[0];
               //generation token
