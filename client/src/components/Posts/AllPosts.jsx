		...
		<div className={stl.wrappPost}>
			<div className={stl.posts}>
				{postsData.map((pst) => {
					return (
						<div
							key={`${pst.title_post}_${pst.created_at}`}
							className={stl.post}>
							<div className={stl.postTitle}>
								{pst.title_post}
							</div>
							<p className={stl.postContent}>
								{pst.content_post}
							</p>

							<div className={stl.fiels}>
								<span className={stl.postLike}>
									Like{' '}
									{Math.floor(Math.random() * (100 - 0)) + 0}
								</span>

								<Link
									className={stl.postAuthor}
									to='user'
									onClick={() => {
